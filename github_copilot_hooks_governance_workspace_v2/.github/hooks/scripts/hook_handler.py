...
def handle_post_tool(payload):
    ...
    if name in {"edit", "create", "Edit", "Write"}:
        return {"additionalContext": "Se modifico un archivo. Si afecta demo_cart, ejecuta sus pruebas focalizadas antes de declarar la tarea terminada."}
    return {}
...
def run_quality_gate():
    cmd = [sys.executable, "-m", "unittest", "discover", "-s", "demo_cart", "-p", "test_*.py"]
...
def handle_agent_stop(payload):
    ...
    return {
        "decision": "block",
        "reason": "Quality gate final fallido. Corrige los tests de demo_cart antes de finalizar. Salida de tests:\n" + output
    }
